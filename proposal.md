> [!NOTE]
> This file (proposal.md) was the originally uploaded _proposal_ for this project. The latest README.md covers the actual results, this here was separated out for the sake of keeping it as an original reference and also for comparing what ended up really happening as planned, what was cut and what was perhaps added.
...
> project name is meant to be a little playful and sound like a game, given contents to follow. It's also intended to capture the iot and realtime aspects, lightly inspired by IFTTT, shortened from iot-right-now -> IoNoW. Aesthetically pleasing, interesting sounding, short-n-sweet, checks all the boxes!
...
# A note on AI
As a general opposer of AI, it will be kept to a minimum in my project. I am plenty well aware of it's positive usecases and astonishing performance in __fitting__ situations, however it's extreme overuse in marketing and general mentality of "Dunno, let's sprinkle some AI on that" is definitely not something I can resonate with. I am using [Brave](https://search.brave.com/) and [DuckDuckGo](https://duckduckgo.com/) (mobile) as my default search engines, which do include AI, occassinaly I also use ChatGPT when I have specialized queries that regular search engines fail to grasp. Code generation however (especially agents) and any writing is not something AI will touch in my projects. For me, with projects I care about, AI can at most be a personalized version of 'Stack Overflow'. For such instances of specific code snippets or API showcases I see no reason to give special disclaimers. As was the case for decades, no one ever disclosed when, where and how they used Stack Overflow. I use ChatGPT in the exact same way (and will prefer real Stack Overflow when results exist). I am optimistic about it's solutions/snippets but trust them just as much (or rather little) as with any random close-to-zero-vote post on Stack Overflow.
